...
// jumlah data
$jumlahData = count(query("SELECT * FROM list3 WHERE judul LIKE '%$keyword%' ORDER BY id DESC"));
...
//query 
$datas = query("SELECT * FROM list3 WHERE judul LIKE '%$keyword%' ORDER BY id DESC LIMIT $awalData, $JumlahDataPerHalaman");
...
<!-- tampilkan data di depan -->
    ...
    <div class="navPage">
            <?php if($halamanAktif > 1) : ?>
                <a href="?page=<?= $halamanAktif -1 ?>&keyword=<?= $keyword ?>">
            ...
            <?php for($i = 1; $i <= $jumlahHalaman; $i++) : ?>
                <?php if( $i == $halamanAktif ) : ?>
                    <a href="?page=<?= $i; ?>&keyword=<?= $keyword ?>" style="font-weight: bold; color: red;"><?= $i; ?></a>
                <?php else : ?>
